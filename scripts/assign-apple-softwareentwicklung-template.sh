#!/usr/bin/env bash
# Assign Apple Softwareentwicklung template on production and park Elementor rendering.
set -euo pipefail
...
PAGE_ID="$(wp post list --post_type=page --name=softwareentwicklung --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
if [[ -z "${PAGE_ID}" ]]; then
  echo "Could not resolve page slug softwareentwicklung" >&2
  exit 1
fi

THEME="$(wp theme path --dir 2>/dev/null || true)"
BACKUP_DIR="${THEME}/.apple-app-page-backups/$(date -u +%Y%m%dT%H%M%SZ)-softwareentwicklung"
mkdir -p "$BACKUP_DIR"
...
wp post meta update "$PAGE_ID" _wp_page_template 'template-apple-softwareentwicklung.php'
...
echo "Assigned Apple Softwareentwicklung template to page ID ${PAGE_ID}"
