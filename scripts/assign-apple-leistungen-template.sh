#!/usr/bin/env bash
# Assign Apple Leistungen template on production and stop Elementor from owning the page.
set -euo pipefail
...
PAGE_ID="$(wp post list --post_type=page --name=leistungen --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
if [[ -z "${PAGE_ID}" ]]; then
  PAGE_ID="$(wp post list --post_type=page --post__in=787 --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
fi
if [[ -z "${PAGE_ID}" ]]; then
  echo "Leistungen page not found" >&2
  exit 1
fi

THEME="$(wp theme path --dir 2>/dev/null || true)"
BACKUP_DIR="${THEME}/.apple-leistungen-backups/$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$BACKUP_DIR"
...
wp post meta update "$PAGE_ID" _wp_page_template 'template-apple-leistungen.php'
...
echo "Assigned Apple Leistungen template to page ID ${PAGE_ID}"
