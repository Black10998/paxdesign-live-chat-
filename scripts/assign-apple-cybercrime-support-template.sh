#!/usr/bin/env bash
# Create / assign Apple Cybercrime Support template on production.
set -euo pipefail
...
PAGE_ID="$(wp post list --post_type=page --name=cybercrime-support --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
if [[ -z "${PAGE_ID}" ]]; then
  PAGE_ID="$(wp post create \
    --post_type=page \
    --post_title='Cybercrime Support' \
    --post_name='cybercrime-support' \
    --post_status=publish \
    --porcelain)"
  echo "Created page cybercrime-support (ID ${PAGE_ID})"
fi

THEME="$(wp theme path --dir 2>/dev/null || true)"
BACKUP_DIR="${THEME}/.apple-cybercrime-backups/$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$BACKUP_DIR"
...
wp post meta update "$PAGE_ID" _wp_page_template 'template-apple-cybercrime-support.php'
...
echo "Assigned Apple Cybercrime Support template to page ID ${PAGE_ID}"
