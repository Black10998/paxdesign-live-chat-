#!/usr/bin/env bash
# Assign Apple Impressum template on production and park Elementor rendering.
set -euo pipefail
...
PAGE_ID="$(wp post list --post_type=page --name=impressum --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
if [[ -z "${PAGE_ID}" ]]; then
  PAGE_ID="$(wp post list --post_type=page --pagename=impressum --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
fi
if [[ -z "${PAGE_ID}" ]]; then
  PAGE_ID=2838
fi
...
BACKUP_DIR="${THEME}/.apple-impressum-backups/$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$BACKUP_DIR"
...
wp post meta update "$PAGE_ID" _wp_page_template 'template-apple-impressum.php'
...
echo "Assigned Apple Impressum template to page ID ${PAGE_ID}"
