#!/usr/bin/env bash
# Assign Apple Advanced Website Systems template on production and park Elementor rendering.
set -euo pipefail
...
PAGE_ID="$(wp post list --post_type=page --name=advanced-website-systems --field=ID --format=ids 2>/dev/null | awk '{print $1}')"
if [[ -z "${PAGE_ID}" ]]; then
  echo "Could not resolve page slug advanced-website-systems" >&2
  exit 1
fi

THEME="$(wp theme path --dir 2>/dev/null || true)"
BACKUP_DIR="${THEME}/.apple-app-page-backups/$(date -u +%Y%m%dT%H%M%SZ)-advanced-website-systems"
mkdir -p "$BACKUP_DIR"
...
wp post meta update "$PAGE_ID" _wp_page_template 'template-apple-advanced-website-systems.php'
...
echo "Assigned Apple Advanced Website Systems template to page ID ${PAGE_ID}"
