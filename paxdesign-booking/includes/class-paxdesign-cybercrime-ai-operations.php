...
class PAXdesign_Cybercrime_AI_Operations {
    ...
    public static function persist_assistant_reply($session_id, $content, $operation = array(), $client_msg_id = '') {
        ...
        $lock_key = 'pax_ccs_asst_lock_' . md5($session_id);
        $waited = 0;
        while ($waited < 8 && get_transient($lock_key)) {
            usleep(25000);
            $waited++;
        }
        ...
        try {
            ...
            $latest_user = self::latest_user_message($session_id);
            if (is_array($latest_user) && !empty($latest_user['id'])) {
                $extra['reply_to'] = absint($latest_user['id']);
            }
     ...
     * @return array<string, mixed>|null
     */
    public static function latest_user_message($session_id) {
        $session_id = sanitize_text_field((string) $session_id);
        if ($session_id === '' || !class_exists('PAXdesign_Message_Store')) {
            return null;
        }
        ...
        return is_array($last) && !empty($last['id']) ? $last : null;
    }
