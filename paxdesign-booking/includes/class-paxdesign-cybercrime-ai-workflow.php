...
class PAXdesign_Cybercrime_AI_Workflow {
    ...
    public static function persist_snapshot($row, $snapshot) {
        ...
        $previous = is_array($payload['ai_workflow'] ?? null) ? $payload['ai_workflow'] : array();
        $unchanged = ((int) ($previous['step'] ?? 0) === $workflow['step'])
            && ((string) ($previous['step_key'] ?? '') === $workflow['step_key'])
            && ((array) ($previous['missing'] ?? array()) === $workflow['missing'])
            && (!empty($previous['can_submit']) === $workflow['can_submit']);
        if ($unchanged) {
            return $row;
        }
        $workflow['updated_at'] = function_exists('current_time') ? current_time('mysql', true) : gmdate('Y-m-d H:i:s');
    ...
    /**
     * One short next-action prompt. Do not recap the case unless asked.
     *
     * @param array<string, mixed> $snapshot
     * @param array<string, mixed> $state
     * @param string               $lang
    ...
    public static function assistant_copy($snapshot, $state, $lang = '', $wanted_submit = false, $user_message = '') {
        ...
        if (self::is_reference_question($user_message)) {
            if ($ref === '') {
                return $lang === 'ar' ? 'سيظهر رقم المرجع بعد فتح البلاغ.' : ($lang === 'de' ? 'Die Referenznummer erscheint nach dem Öffnen des Falls.' : 'The reference number appears after the case is opened.');
            }
            return $ref;
        }
        if (self::is_status_question($user_message)) {
            $status_line = self::short_status_line($snapshot, $state, $lang);
            $prompt = $next_key !== '' ? self::next_prompt($next_key, $lang) : self::submit_prompt($lang);
     ...
     * @return string
     */
    public static function next_prompt($key, $lang) {
        $lang = self::normalize_lang($lang);
        $prompts = array(
            'full_name' => array(
            ...
            'identity_document' => array(
                'en' => 'Upload your identity document with the + button.',
                'de' => 'Laden Sie das Ausweisdokument über die +-Taste hoch.',
                'ar' => 'ارفع وثيقة الهوية من زر +.',
            ),
            'identity_accuracy' => array(
                'en' => 'Confirm the identity details are correct.',
                'de' => 'Bestätigen Sie, dass die Identitätsangaben stimmen.',
                'ar' => 'أكد أن بيانات الهوية صحيحة.',
            ),
            'incident_type' => array(
                'en' => 'What type of incident is this?',
                'de' => 'Um welche Art von Vorfall handelt es sich?',
                'ar' => 'ما نوع الحادثة؟',
            ),
            'incident_date' => array(
            ...
            'platforms' => array(
                'en' => 'Which platform was affected?',
                'de' => 'Welche Plattform war betroffen?',
                'ar' => 'ما المنصة المتأثرة؟',
            ),
            'description' => array(
                'en' => 'Briefly describe what happened.',
                'de' => 'Beschreiben Sie kurz, was passiert ist.',
                'ar' => 'صف باختصار ما حدث.',
            ),
            'evidence_files' => array(
                'en' => 'Upload evidence with the + button.',
                'de' => 'Laden Sie Beweise über die +-Taste hoch.',
                'ar' => 'ارفع الدليل من زر +.',
            ),
            'declarations' => array(
                'en' => 'Type “submit report” to send.',
                'de' => 'Schreiben Sie „Bericht absenden“ zum Senden.',
                'ar' => 'اكتب «أرسل البلاغ» للإرسال.',
            ),
        );
     ...
     * @return string
     */
    public static function submit_prompt($lang) {
        $lang = self::normalize_lang($lang);
        if ($lang === 'ar') {
            return 'اكتب «أرسل البلاغ» للإرسال.';
        }
        if ($lang === 'de') {
            return 'Schreiben Sie „Bericht absenden“ zum Senden.';
        }
        return 'Type “submit report” to send.';
    }
     ...
     * @return bool
     */
    public static function is_reference_question($text) {
        $normalized = self::normalize_text($text);
        if ($normalized === '') {
            return false;
        }
        return (bool) preg_match('/(?:reference(?: number)?|case number|ticket number|رقم المرجع|رقم البلاغ|رقم الطلب|aktenzeichen|referenznummer)/u', $normalized);
    }
     ...
     * @return bool
     */
    public static function is_status_question($text) {
        $normalized = self::normalize_text($text);
        if ($normalized === '') {
            return false;
        }
        if (self::is_reference_question($text)) {
            return false;
        }
        return (bool) preg_match('/(?:\bstatus\b|current status|what(?:\'s| is) (?:the )?status|ما (?:هو )?الوضع|ما حالة|شو الوضع|الحالة الحالية|statusabfrage|aktueller status)/u', $normalized);
    }
     ...
     * @return string
     */
    private static function short_status_line($snapshot, $state, $lang) {
        $lang = self::normalize_lang($lang);
        $step = (string) ($snapshot['step_label'] ?? '');
        if ($lang === 'ar') {
            return $step !== '' ? ('الخطوة الحالية: ' . $step . '.') : 'البلاغ قيد الإكمال.';
        }
        if ($lang === 'de') {
            return $step !== '' ? ('Aktueller Schritt: ' . $step . '.') : 'Der Fall wird vervollständigt.';
        }
        return $step !== '' ? ('Current step: ' . $step . '.') : 'The report is being completed.';
    }
    ...
    public static function submitted_copy($success_message, $reference, $lang = '') {
        ...
        if ($lang === 'de') {
            return 'Der Bericht wurde gesendet.';
        }
        return 'The report was submitted.';
    }
