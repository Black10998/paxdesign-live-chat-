...
(function () {
  ...
  function setHeaderCtaLabel(btn, className, text) {
    if (!btn) return;
    var label = btn.querySelector('.' + className);
    if (!label) {
      btn.textContent = '';
  ...
  var syncingHeaderAuth = false;

  function syncHeaderAuthCtas() {
    if (syncingHeaderAuth) return;
  ...
  function watchHeaderAuthSanitizer() {
    ...
    var target = document.getElementById('pdx-auth-bar') || document.getElementById('dtr-main-header') || document.body;
    if (!target) return;
