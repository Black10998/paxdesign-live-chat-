...
if ( ! function_exists( 'navein_apple_header_desktop_cascade_footer' ) ) {
	function navein_apple_header_desktop_cascade_footer() {
		...
		echo '<script id="navein-apple-header-desktop-cascade-js">'
			...
			. '});'
			. '}'
			. 'function scheduleReset(){resetDesktopHeaderAuth();setTimeout(resetDesktopHeaderAuth,0);'
			. 'setTimeout(resetDesktopHeaderAuth,50);setTimeout(resetDesktopHeaderAuth,250);}'
			. 'if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",scheduleReset);}'
			. 'else{scheduleReset();}'
			. 'window.addEventListener("load",resetDesktopHeaderAuth);'
			. 'window.addEventListener("resize",resetDesktopHeaderAuth,{passive:true});'
			. '})();'
			. '</script>' . "\n";
	}
}
