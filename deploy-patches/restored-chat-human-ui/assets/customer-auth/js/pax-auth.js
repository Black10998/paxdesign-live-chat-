...
(function () {
  ...
  function clearPendingSocialError() {
    ...
    try { sessionStorage.removeItem('pdx_github_error'); } catch (e2) {}
  ...
  function captureAppleErrorFromUrl(params) {
    ...
    if (params.get('pdx_apple') !== 'error') return;
    ...
    try { sessionStorage.setItem('pdx_apple_error', appleMsg); } catch (err2) {}
  }

  function clearPendingAppleErrorFromUrl(params) {
    if (!params) return;
    if (params.get('pdx_apple') !== 'error' && params.get('pdx_github') !== 'error') return;
    setTimeout(function () {
