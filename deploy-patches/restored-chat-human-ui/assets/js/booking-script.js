...
(function($) {
    ...
    var siteHeaderBottomCache = 0;
    var siteHeaderBottomAt = 0;

    function getSiteHeaderBottom() {
        var now = Date.now();
        if (siteHeaderBottomCache > 0 && (now - siteHeaderBottomAt) < 400) {
            return siteHeaderBottomCache;
        }
        var selectors = [
            '#dtr-main-header',
            '#dtr-header-global',
        ...
        siteHeaderBottomCache = Math.max(Math.ceil(maxBottom), 48);
        siteHeaderBottomAt = now;
        return siteHeaderBottomCache;
    }
    ...
    function syncChatOrganizerPanels() {
        if (!$in('.paxdesign-booking-mode-switch').length) return;
    ...
    function applyCompactWidgetFrame() {
        ...
        if (!mobile) {
            var deskH = keyboardOpen
                ? Math.max(220, Math.min(COMPACT_HEIGHT, box.height - 8))
                : COMPACT_HEIGHT;
            $widget.css({
                position: 'fixed',
                top: 'auto',
    ...
    function openWidget() {
        ...
        if (hasChat) {
            syncChatOrganizerPanels();
        } else {
            root().toggleClass('paxdesign-mobile-chat-mode', currentWidgetMode === 'chat' && mobile);
            root().toggleClass('paxdesign-frame-managed', mobile);
        }
