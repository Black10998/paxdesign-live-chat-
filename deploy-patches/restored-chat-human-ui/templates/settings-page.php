...
$github_oauth_client_id = get_option('paxdesign_github_oauth_client_id', '');
$github_oauth_secret_set = trim((string) get_option('paxdesign_github_oauth_client_secret', '')) !== '';
$github_web_callback_url = class_exists('PAXdesign_Auth_GitHub')
    ? PAXdesign_Auth_GitHub::web_callback_url()
      ...
      <div class="ps-main">
        ...
        <section class="ps-card" aria-labelledby="ps-section-email">
          ...
          <div class="ps-card-body">
            ...
            <div class="ps-divider"></div>
            <h3 class="ps-subheading">Website — Sign in with GitHub (Web OAuth)</h3>
            <p class="ps-hint ps-hint--block">Uses a GitHub OAuth App. Put the Client ID and Client Secret in GitHub Actions secrets <code>PAX_GITHUB_OAUTH_CLIENT_ID</code> / <code>PAX_GITHUB_OAUTH_CLIENT_SECRET</code>, or enter them here. Homepage URL: <?php echo esc_html(home_url('/')); ?></p>

            <div class="ps-field">
              <label class="ps-label" for="paxdesign_github_oauth_client_id">GitHub OAuth Client ID</label>
              <input type="text" id="paxdesign_github_oauth_client_id" name="paxdesign_github_oauth_client_id"
                     class="ps-input" value="<?php echo esc_attr($github_oauth_client_id); ?>" placeholder="Ov23li…">
            </div>

            <div class="ps-field">
              <label class="ps-label" for="paxdesign_github_oauth_client_secret">GitHub OAuth Client Secret</label>
              <input type="password" id="paxdesign_github_oauth_client_secret" name="paxdesign_github_oauth_client_secret"
                     class="ps-input" value="" autocomplete="new-password"
                     placeholder="<?php echo $github_oauth_secret_set ? '••••••••  (leave blank to keep)' : 'github_oauth client secret'; ?>">
              <span class="ps-hint"><?php echo $github_oauth_secret_set ? 'A client secret is already stored. Leave blank to keep it.' : 'Not set yet. Paste the OAuth App client secret.'; ?></span>
            </div>

            <div class="ps-field">
              <label class="ps-label">Authorization callback URL (register in the GitHub OAuth App)</label>
              <input type="text" class="ps-input" readonly value="<?php echo esc_attr($github_web_callback_url); ?>"
                     onclick="this.select();" style="cursor:pointer">
              <span class="ps-hint">GitHub → Settings → Developer settings → OAuth Apps → Authorization callback URL.</span>
            </div>
