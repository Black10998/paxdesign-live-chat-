...
class PAXdesign_Auth_REST {
    ...
    public static function register_routes() {
        ...
        register_rest_route(self::NS, '/auth/github/start', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array(__CLASS__, 'github_web_start'),
            'permission_callback' => $pub,
        ));
        register_rest_route(self::NS, '/auth/github/callback', array(
            'methods'             => array(WP_REST_Server::READABLE, WP_REST_Server::CREATABLE),
            'callback'            => array(__CLASS__, 'github_web_callback'),
            'permission_callback' => $pub,
        ));
    ...
    public static function github_web_start(WP_REST_Request $request) {
        if (!defined('DONOTCACHEPAGE')) {
            define('DONOTCACHEPAGE', true);
        }
        nocache_headers();
        ...
        $limited = self::rate_limit('login');
        if ($limited) {
            wp_safe_redirect(PAXdesign_Auth_GitHub::web_error_redirect_url(
                __('Too many attempts. Please try again later.', 'paxdesign-booking')
            ));
            exit;
        }

        $url = PAXdesign_Auth_GitHub::web_begin_authorization($request);
        if (is_wp_error($url)) {
            wp_safe_redirect(PAXdesign_Auth_GitHub::web_error_redirect_url($url->get_error_message()));
            exit;
        }
    ...
    public static function github_web_callback(WP_REST_Request $request) {
        if (!defined('DONOTCACHEPAGE')) {
            define('DONOTCACHEPAGE', true);
        }
        nocache_headers();
        $redirect = PAXdesign_Auth_GitHub::web_handle_callback($request);
