...
(function () {
  ...
  function init() {
    ...
    /* ── Session gates (avoid REST 401 noise on public frontend) ── */
    function canAccessAuthenticatedDock() {
      return !!(window.PDXAuth && window.PDXAuth.isLoggedIn && window.PDXAuth.isLoggedIn());
    }

    function canAccessInfrastructure() {
      if (!canAccessAuthenticatedDock()) return false;
      var u = window.PDXAuth.getUser ? window.PDXAuth.getUser() : null;
      return !!(u && u.is_admin);
    }
    ...
    /* ── Load AI memory ───────────────────────────────────── */
    if (C.aiMemory && canAccessAuthenticatedDock()) {
