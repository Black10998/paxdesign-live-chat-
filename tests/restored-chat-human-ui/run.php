...
assert_true(strpos($live, 'function reject_guest_chat') !== false, 'PHP rejects guest chat handlers');
assert_true(strpos($live, "'code'    => 'login_required'") !== false, 'PHP guest reject uses login_required');
assert_true(preg_match('/function handle_chat_nonce\(\)\s*\{[^}]*reject_guest_chat/s', $live) === 1, 'nonce handler requires login');
assert_true(preg_match('/function handle_user_attach\(\)\s*\{[^}]*reject_guest_chat/s', $live) === 1, 'attach handler requires login');
assert_true(preg_match('/function handle_disconnect\(\)\s*\{[^}]*reject_guest_chat/s', $live) === 1, 'disconnect handler requires login');
...
assert_true(strpos($css, 'paxdesign-booking-chat-auth-gate-card') !== false, 'widget auth gate uses centered card layout');
assert_true(strpos($css, 'auth-actions--with-social') === false, 'Sign In stays black when social buttons are shown');
assert_true(strpos($css, 'paxdesign-chat-auth-locked .paxdesign-booking-close') !== false, 'auth gate keeps close button visible');
assert_true(strpos($css, 'paxdesign-chat-auth-locked .paxdesign-booking-header-content') !== false, 'auth gate hides header content but not close');
...
assert_true($bookingJs !== false && strpos($bookingJs, 'resetGuestAuthGateUi') !== false, 'booking-script resets auth gate when chat closes');
